...
import se.idega.util.Constants;
...
public class CitizenAccountApplication extends CommuneBlock {
	...
	private void submitSimpleForm(final IWContext iwc) {
		...
		try {
			...
			if (user == null || !citizenLivesInNacka(iwc, user)) {
				...
				PKILoginBusinessBean loginBusiness = PKILoginBusinessBean.createNBSLoginBusiness();
				LoggedOnInfo info = loginBusiness.getLoggedOnInfo(iwc.getSession());
	...
	private void addSimpleInputs(final Table table, final IWContext iwc) {
		...
		PKILoginBusinessBean NBSLBusiness = PKILoginBusinessBean.createNBSLoginBusiness();
		String unlockAction = "nbs_unlock_action";
		if(iwc.getParameter(unlockAction)!= null){
			NBSLBusiness.logOutUser(iwc);
		}

		LoggedOnInfo info = PKILoginBusinessBean.getLoggedOnInfo(iwc);
		if(info !=null){
			String persID = info.getUser().getPersonalID();
			if(persID!= null){
				...
				unlockLink.addParameter(Constants.PARAMETER_PERSONAL_ID, persID);
			...
			String ssn = iwc.getParameter(Constants.PARAMETER_PERSONAL_ID);
			if(ssn!=null){
